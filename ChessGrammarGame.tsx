...
const QUESTIONS: Question[] = [
  ...
  { id: 51, text: 'Ինչպե՞ս կլինի իսպաներեն «Վատ»:', options: ['Bien', 'Mal', 'Peor'], correct: 'Mal', explanation: '"Mal" es mal.', explanationAm: '«Mal» նշանակում է վատ:' },
  { id: 52, text: 'Ինչպե՞ս կլինի իսպաներեն «Արագ»:', options: ['Lento', 'Rápido', 'Pronto'], correct: 'Rápido', explanation: '"Rápido" es rápido.', explanationAm: '«Rápido» նշանակում է արագ:' },
  { id: 53, text: 'Ինչպե՞ս կլինի իսպաներեն «Դանդաղ»:', options: ['Rápido', 'Lento', 'Suave'], correct: 'Lento', explanation: '"Lento" es lento.', explanationAm: '«Lento» նշանակում է դանդաղ:' },
  { id: 54, text: 'Ինչպե՞ս կլինի իսպաներեն «Ուժեղ»:', options: ['Débil', 'Fuerte', 'Duro'], correct: 'Fuerte', explanation: '"Fuerte" es fuerte.', explanationAm: '«Fuerte» նշանակում է ուժեղ:' },
  { id: 55, text: 'Ինչպե՞ս կլինի իսպաներեն «Թույլ»:', options: ['Fuerte', 'Débil', 'Blando'], correct: 'Débil', explanation: '"Débil" es débil.', explanationAm: '«Débil» նշանակում է թույլ:' },
  { id: 56, text: 'Ինչպե՞ս կլինի իսպաներեն «Նոր»:', options: ['Viejo', 'Nuevo', 'Joven'], correct: 'Nuevo', explanation: '"Nuevo" es nuevo.', explanationAm: '«Nuevo» նշանակում է նոր:' },
  { id: 57, text: 'Ինչպե՞ս կլինի իսպաներեն «Հին»:', options: ['Nuevo', 'Viejo', 'Antiguo'], correct: 'Viejo', explanation: '"Viejo" es viejo.', explanationAm: '«Viejo» նշանակում է հին:' },
  { id: 58, text: 'Ինչպե՞ս կլինի իսպաներեն «Տաք»:', options: ['Frío', 'Caliente', 'Tibio'], correct: 'Caliente', explanation: '"Caliente" es caliente.', explanationAm: '«Caliente» նշանակում է տաք:' },
  { id: 59, text: 'Ինչպե՞ս կլինի իսպաներեն «Սառը»:', options: ['Caliente', 'Frío', 'Hielo'], correct: 'Frío', explanation: '"Frío" es frío.', explanationAm: '«Frío» նշանակում է սառը:' },
  { id: 60, text: 'Ինչպե՞ս կլինի իսպաներեն «Գեղեցիկ»:', options: ['Feo', 'Hermoso', 'Lindo'], correct: 'Hermoso', explanation: '"Hermoso" es hermoso.', explanationAm: '«Hermoso» նշանակում է գեղեցիկ:' }
];
...
export default function ChessGrammarGame() {
  ...
  const makeComputerMove = (currentGame: Chess) => {
    ...
    // Ensure it's black's turn for the computer to move
    if (gameCopy.turn() === 'w') {
      const fen = gameCopy.fen().replace(/ [wb] /, ' b ');
      gameCopy.load(fen);
    }
    ...
    if (moves.length > 0) {
      ...
      setGame(gameCopy);
      setMoveStatus('Սխալ պատասխան: Մրցակիցը կատարեց իր քայլը: Սեղմիր «Հաջորդ հարցը» կոճակը:');
    } else {
      setGame(gameCopy);
    }
  };
  ...
  const handleCellClick = (square: string) => {
    ...
    // Ensure it's white's turn for the user to move
    const gameCopy = new Chess(game.fen());
    if (gameCopy.turn() === 'b') {
      const fen = gameCopy.fen().replace(/ [wb] /, ' w ');
      gameCopy.load(fen);
    }
    ...
    try {
      ...
      if (move) {
        ...
        setMoveStatus('Քայլը կատարված է: Սեղմիր «Հաջորդ հարցը» կոճակը:');
      } else {
        const piece = gameCopy.get(square as any);
        if (piece && piece.color === 'w') {
          setSelectedSquare(square);
  ...
  const handleOptionClick = (option: string) => {
    if (feedback?.isCorrect || isGameOver) return;
    ...
    if (isCorrect) {
      setMoveCredit(true);
      setMoveStatus('Ճիշտ է! Այժմ կատարիր քո քայլը:');
    } else {
      setMoveCredit(false);
      setMoveStatus('Սխալ պատասխան: Մրցակիցը պատրաստվում է քայլ կատարել...');
              ...
              <div className="space-y-3">
                {currentQuestion.options.map((option) => (
                  <button
                    ...
                    disabled={feedback?.isCorrect || isGameOver}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        {((feedback.isCorrect && !moveCredit) || !feedback.isCorrect) && !isGameOver && (
                          <button
                            onClick={() => {
                            ...
                            className="mt-4 w-full py-3 bg-white/20 hover:bg-white/30 border border-white/30 rounded-xl font-bold flex items-center justify-center gap-2 transition-all"
                          >
                            Հաջորդ հարցը
